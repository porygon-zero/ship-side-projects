...
Parallelize read-only discovery across disjoint concerns. Serialize overlapping edits, generated files, lockfiles, schemas, migrations, behavior-defining decisions, approval transitions, final documentation reconciliation, commit creation, push, PR creation, and other external mutations. Assign one writer per overlapping artifact and have the lead integrate findings. The lead owns the accepted-increment closeout sequence and opens the PR only after every merge-required artifact is committed. Bound each assignment by a concrete question, deliverable, stopping condition, and proportionate effort budget.
